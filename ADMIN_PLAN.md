...
| 페이지 | 우선순위 | 핵심 기능 |
|--------|----------|-----------|
| **대시보드 홈** | 🔴 1순위 | 요약 카드, 최근 문의, 인기 사진 |
| **회원 관리** | 🔴 1순위 | 목록/검색, 역할 변경, 삭제 |
| **사진 관리** | 🔴 1순위 | 전체 사진 그리드, 삭제 |
| **문의 관리** | 🔴 1순위 | 전체 문의 목록, 읽음/삭제 |
| **통계** | 🟡 2순위 | 업로드 추이, 인기 사진 TOP 10, 무드 분포 |
| **시리즈 관리** | 🟢 3순위 | 전체 시리즈 목록, 삭제 |
| **시스템 설정** | 🟢 3순위 | 메일 설정 상태, Rate Limit 확인 |
...
### Step 1 — 기반 작업
- [ ] Admin Backend: DB 연결 설정 (`application-prod.yml` → happiness-app 동일 PostgreSQL)
- [ ] Admin Backend: happiness-app 엔티티 동기화 (Member, Photo, Inquiry, Series, SeriesPhoto)
- [ ] Admin Backend: JWT 인증 필터 (happiness-app과 동일 secret으로 검증)
- [ ] Admin Frontend: 로그인 페이지 React 전환 + AuthContext 구성
- [ ] Admin Frontend: Sidebar 레이아웃 컴포넌트

### Step 2 — 핵심 CRUD 페이지 (1순위)
- [ ] `GET /api/admin/stats/summary` + DashboardPage 카드
- [ ] MemberListPage + 역할 변경/삭제
- [ ] PhotoListPage + 삭제
- [ ] InquiryListPage + 필터/삭제

### Step 3 — 통계 (2순위)
- [ ] `GET /api/admin/stats/daily` + 꺾은선 그래프
- [ ] `GET /api/admin/stats/top-photos` + 인기 사진 테이블
- [ ] 무드 분포 / 촬영 종류 분포 차트

### Step 4 — 나머지
- [ ] SeriesListPage
- [ ] SystemPage (메일/Rate Limit 상태)
- [ ] 페이지네이션 공통 컴포넌트
